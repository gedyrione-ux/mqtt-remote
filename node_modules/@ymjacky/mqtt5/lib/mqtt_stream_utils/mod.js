import { ConnectionClosed, ConnectionReset } from "../client/error.js";
import * as MqttUtils from "../mqtt_utils/mod.js";
export async function readMqttBytes(reader) {
  try {
    const receiveBytes = [];
    ...
    if (remainingLength.number > 0) {
      readValue = await reader.read(new Uint8Array(remainingLength.number));
      if (readValue.done) {
        throw new ConnectionClosed();
      }
      receiveBytes.push(...readValue.value);
